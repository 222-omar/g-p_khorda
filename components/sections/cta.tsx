...
export function CTA() {
    ...
    return (
        <section className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-900/50">
            <motion.div className="max-w-4xl mx-auto relative" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6 }}>
                    ...
                    <div className="relative z-10">
                        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                            ...
                            <motion.h2 variants={staggerItem} className="text-3xl md:text-5xl font-black text-white mb-6 leading-tight">
                                {dict.cta.title}
                                <br />
                                {'مع '}<span className="bg-gradient-to-r from-primary to-green-400 bg-clip-text text-transparent">{dict.cta.titleHighlight}</span>{'؟'}
                            </motion.h2>

                            <motion.p variants={staggerItem} className="text-slate-400 max-w-lg mx-auto mb-10 text-lg">
                                {dict.cta.subtitle}
                            </motion.p>

                            <motion.div variants={staggerItem} className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                <Link href={user ? '/dashboard' : '/register'}>
                                    <motion.button whileHover={{ scale: 1.04, boxShadow: '0 8px 30px rgba(22,163,74,0.4)' }} whileTap={{ scale: 0.97 }} className="bg-primary hover:bg-primary-600 text-white px-10 py-4 rounded-xl font-bold shadow-lg shadow-primary/30 transition-colors flex items-center gap-2 text-lg">
                                        {user ? dict.cta.browseShop : dict.cta.registerFree}
                                        <ArrowRight size={20} className="rtl:rotate-180" />
                                    </motion.button>
                                </Link>
                                <Link href={user ? '/sell' : '/login?redirect=/sell'}>
                                    <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="border-2 border-slate-600 hover:border-primary text-slate-300 hover:text-white px-10 py-4 rounded-xl font-bold transition-all">
                                        {dict.cta.addFirstListing}
                                    </motion.button>
                                </Link>
                            </motion.div>
                        </motion.div>
